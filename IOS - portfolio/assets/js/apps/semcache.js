...
(function semcache() {
  ...
  async function ask(q) {
    ...
    try {
      ...
      var msg = e && e.handled ? e.message : 'Live AI offline — run the dev server or deploy to enable embeddings.';
      setStatus('⚠ ' + msg, 'err');
  ...
  // iOS shell lifecycle: focus the input when the Semantic Cache app opens.
  document.addEventListener('ios:appopen', function (e) {
    if (!e.detail || e.detail.id !== 'semcache') return;
    if (!el.q) bind();
    if (el.q) { try { el.q.focus(); } catch (_) {} }
  });
})();
