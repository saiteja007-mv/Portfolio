...
(function textsql() {
  ...
  function firstOpen() {
    if (started) return;
    started = true;
    loadSample();              // lazy-loads sql.js on first open, then seeds Sample DB
  }
  function init() {
    if (!bind()) return;
    // iOS shell: lazy-init on first time the Text-to-SQL sheet opens (no window.WM).
    document.addEventListener('ios:appopen', function (e) {
      if (e && e.detail && e.detail.id === 'textsql') firstOpen();
    });
    // Fallback: if a user interacts before any open event fired, init then too.
    if (el.gen) el.gen.addEventListener('click', firstOpen);
    if (el.run) el.run.addEventListener('click', firstOpen);
  }
