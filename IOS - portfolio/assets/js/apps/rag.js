/* ══════════════════════════════════════════════════════════════════
   HYBRID SEARCH RAG — iOS 26 app (#app-hybridrag)
   ...
   Logic unchanged from the Win95 build; rebound to iOS ids + lifecycle events.
═══════════════════════════════════════════════════════════════════ */
(function rag() {
  'use strict';
  ...
  // ── embeddings / llm ────────────────────────────────────────────────
  var OFFLINE = 'Backend unreachable — run `node dev-server.js` (or deploy) to enable live AI.';
  async function postJSON(url, body) {
    var r;
    try {
      r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    } catch (netErr) { throw new Error(OFFLINE); }
    var d = await r.json().catch(function () { return {}; });
    return { r: r, d: d };
  }
  async function embedMany(texts) {
    var res = await postJSON('/api/embed', { inputs: texts });
    if (!res.r.ok || !res.d.vectors) { throw new Error(res.d.error || 'embed failed'); }
    return res.d.vectors.map(l2);
  }
  async function answer(q, ctx) {
    ...
    var user = 'Context:\n' + ctx + '\n\nQuestion: ' + q;
    var res = await postJSON('/api/complete', { messages: [{ role: 'system', content: sys }, { role: 'user', content: user }], max_tokens: 320 });
    if (!res.r.ok || !res.d.reply) { throw new Error(res.d.error || 'LLM failed'); }
    return String(res.d.reply).replace(/\*\*/g, '').replace(/__/g, '');
  }
  ...
  function init() {
    bind();
    // iOS lifecycle: focus the question field when the Hybrid RAG app opens.
    document.addEventListener('ios:appopen', function (e) {
      if (!e.detail || e.detail.id !== 'hybridrag') return;
      if (!el.pane && !bind()) return;
      try { (ready ? el.q : el.file).focus(); } catch (_) {}
    });
  }
