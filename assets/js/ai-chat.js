/* ══════════════════════════════════════════════════════════════════
   ASK AI ABOUT ME — chat UI for the win-ai window.
   Talks to the serverless proxy at /api/chat (Gemini 2.0 Flash).
...
(function aiChat() {
  ...
  function init() {
    ...
    // Seed the greeting the first time the window is opened.
    if (window.WM && typeof window.WM.open === 'function') {
      var origOpen = window.WM.open.bind(window.WM);
      window.WM.open = function (winId) {
        origOpen(winId);
        if (winId === 'win-ai') {
          seed();
      ...
      seed(); // fallback: seed immediately
    }
  }
